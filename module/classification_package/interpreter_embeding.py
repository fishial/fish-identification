...
from module.classification_package.src.model import EmbeddingModel


class EmbeddingClassifier:
    def __init__(self, model_path, data_set_path, device='cpu', THRESHOLD=4.76, resnet=models.resnet18()):
        self.device = device
        ...
        self.model = EmbeddingModel(resnet, 512, 200)
        ...
        self.data_base = self.__clear_data(torch.load(data_set_path).to(device), 200)
    ...
    def inference(self, image, top_k=6):
        ...
        if len(topest) > 0:
            ...
            my_dict = [[i[0], [iii[0] for iii in topest[:top_k]].count(i[0]), i[1]] for i in topest[:top_k]]
            my_dict = sorted(my_dict, key=lambda x: x[1], reverse=True)
            my_dict = [[match[0], match[2]] for match in my_dict]
            ...
            my_dict = [[i[0], [iii[0] for iii in to_hell[:top_k]].count(i[0]), i[1]] for i in to_hell[:top_k]]
            my_dict = sorted(my_dict, key=lambda x: x[1], reverse=True)
            my_dict = [[match[0], match[2]] for match in my_dict]
    ...
    def __get_confs(self, my_dict):
        ...
        my_dict = [[i[0], round(final_conf[idx], 3)] for idx, i in enumerate(my_dict)]
    ...
    def batch_inference(self, imgs, top_k=8):
        ...
        for dump in dump_embed:
            ...
            if len(topest) > 0:
                ...
                outputs.append([[my_dict[0][0], 0.01]])
    ...
    def __clear_data(self, data_base, emb_vec):
        ...
        for class_id, i in enumerate(data_base):
            ...
            for k_id, k in enumerate(tmp_distances):
                if k / mean_per_class > 1.35 and len(tmp_distances) > 30:
